...
local mdns_resolver = require "mdns_resolver"
...
local function get_bridge_endpoint_config(bridge)
  ...
  log.info_with({hub_logs = true}, string.format(
    ...
    (host_source == "serial-mdns") and (", serial=" .. serial .. " -- mDNS resolution pending") or ""))
...
local function get_bridge_config(bridge, opts)
  ...
  -- When the host is a .local mDNS hostname, cosock cannot resolve it (no nss-mdns on
  -- the hub). Attempt raw mDNS A-record resolution via 224.0.0.251:5353 and cache the
  -- result as BRIDGE_HOST so subsequent calls use the discovered-ip path without retrying.
  if type(endpoint.host) == "string" and endpoint.host:match("%.local$") then
    local resolved = mdns_resolver.resolve(endpoint.host)
    if resolved then
      bridge:set_field(fields.BRIDGE_HOST, resolved, { persist = true })
