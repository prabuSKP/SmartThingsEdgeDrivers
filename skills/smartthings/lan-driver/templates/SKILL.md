...
11. Include a final validation checklist in the generated output, and run it **before packaging**: (a) **Lua syntax check every module** — `luac -p src/**/*.lua` (or any Lua parser) must pass for *all* files. **A single unclosed paren in one module crashes the entire driver on load** (`require` throws → `init.lua` errors → no device appears at all). The most common offender is a multi-line `log.info_with({...}, string.format(...))` closed with one `)` instead of `))`. If `luac` is unavailable, at minimum verify parentheses balance in every file. (b) **confirm no `.lua` exists outside `src/`** (`find . -name '*.lua' -not -path './src/*'` must return nothing); (c) compare mapper/`profile=` names to profile YAML names; (d) confirm startup/discovery logs on the hub.
12. **Lua local-function ordering in `src/vendor/sync.lua`.** Define every `local function` helper **before** anything that calls it — this applies **helper-to-helper, not just helpers-before-public**. Lua resolves names lexically at compile time: any call to a `local function` that textually precedes its declaration compiles as a global lookup (`_ENV["name"]`), which is `nil` at runtime. The error is caught by the `pcall` wrapper in the lifecycle/poll path, so the driver stays alive but **sync fails silently — the bridge comes online yet no child devices are ever created** ("hub devices not visible"). The classic miss: putting all helpers above the public `sync.XXX` functions but still ordering two helpers wrong among themselves (e.g. `ensure_child_device` calling `enqueue_child_create` that is defined a few lines *below* it). Fix: topologically order helpers so each callee appears above its first caller; public functions last. A header comment claiming "all locals defined before use" is **not** proof — the `edge-validation` skill's **Stage 1b runtime-nil lint** (`scripts/check-lua-nils.js`, which also catches undefined-call and pcall-misuse) is what verifies it, and `luac -p` will **not** catch this (it is valid syntax). Also, if a helper is exported as `sync.prime_cursor`, always call it as `sync.prime_cursor(...)` — a bare `prime_cursor(...)` is a different (global) lookup that also resolves to `nil`.
...
20. **The inventory sync MUST create children, not just map them.** `mapper.map_device` only classifies a device — it never creates anything. For **every in-scope mapped device**, `sync_bridge_inventory` MUST call `ensure_child_device(...)` (→ `enqueue_child_create`), and MUST end by calling `sync.drain_create_queue(...)` (which calls `driver:try_create_device{ type = "EDGE_CHILD", parent_device_id = bridge.id, parent_assigned_child_key = key, profile = mapped.profile, … }`). A sync that logs `"… mapped as …"` with **no following `try_create_device` (EDGE_CHILD)** is broken: the bridge connects and `/api/devices` returns 200, but **zero child devices are created** and nothing appears in the app. Never "map and move on." See `bridge/hub-event-sync` → §1.
21. **`driver.datastore.<field>` is copy-in / copy-out — in-place mutation does NOT persist.** Reading a datastore field returns a serialized **snapshot**, so `table.insert(driver.datastore.queue, x)` is lost and `#` reports the *committed* length (it reads **0 right after an insert** — the tell-tale sign). To persist any datastore-backed structure (create queue, in-flight map, pending caches): read a **materialized local copy**, mutate the copy, then **reassign the whole table back** (`driver.datastore.queue = copy`). Red flag in logs: an item "queued"/"enqueued" but the size prints `0`, and `try_create_device` never fires — the queue is never persisting, so **no child devices are ever created**. See `bridge/hub-event-sync`.
22. **`sleep` is `socket.sleep`, never `cosock.sleep`; do NOT hand-roll a periodic loop in `init.lua`.** `cosock` has **no `sleep`** — `socket.sleep(n)` (from `require "cosock.socket"`) is the only correct form. `cosock.sleep(n)` is `nil` → `attempt to call a nil value (field 'sleep')`, and when it sits inside a `cosock.spawn(...)` thread started in `init.lua` it **crashes the whole driver at startup** (the runner dies → `BACKOFF` → the bridge never loads, nothing is discoverable). For recurring work, **do not** write `cosock.spawn(function() while true do … socket.sleep(n) end end)` in `init.lua`; schedule it the way the reference driver does — on the bridge device's thread (`bridge.thread:call_on_schedule(interval, fn, name)`, cancel with `bridge.thread:cancel_timer`) in `sync.start_poll_timer`, or via `driver:call_on_schedule(...)` for an init-level scan. The reference never calls `cosock.sleep` and never spawns a periodic loop in `init.lua`. The `edge-validation` Stage 2 grep flags both `cosock.sleep` and `cosock.spawn` in `init.lua`.
23. **Lua comments are `--`, NEVER `//`.** Do not emit C/JavaScript-style `//` comments (a common cross-language slip on section dividers like `// SWITCH COMMANDS`). In Lua `//` is the integer floor-division operator, so a line-leading `//` is a **syntax error** — and if it lands anywhere in `init.lua` (even a divider comment), the file fails to parse, `init.lua` never loads, the driver crash-loops, and **no device card appears during the scan** (v42 shipped 31 such lines across `init.lua`/`sync.lua`/`commands.lua`/`lifecycle.lua`). Every comment — inline or block-divider — must use `--` (or `--[[ … ]]`). `luac -p` and the `edge-validation` Stage 2 `//`-guard both flag this.
24. **`supported_capabilities` is a Lua array in the `Driver(...)` constructor — NOT files.** Register it inline, e.g. `supported_capabilities = { capabilities.switch, capabilities.switchLevel, capabilities.refresh, … }` (a flat list of capability **objects**, scoped to the in-scope kinds). Do **not** create a `src/supported_capabilities/` directory, per-capability `*.yml` files, or any file artifact for capabilities (v42 invented a bogus `src/supported_capabilities/*.yml` tree). Capabilities are declared in code; only **profiles** are YAML, and they live in `profiles/*.yml`, never under `src/`.
